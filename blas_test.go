...
func BenchmarkGoDense64(b *testing.B) {
	...
	dense_a := mat.NewDense(M, K, in1)
	dense_b := mat.NewDense(K, N, in2)
	out := make([]float64, M * N)
	dense_c := mat.NewDense(M, N, out)
	b.SetBytes(M*K*N)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		dense_c.Mul(dense_a, dense_b)
	}
}
 
func BenchmarkDenseBlas64(b *testing.B) {
